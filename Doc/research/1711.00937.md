...
Source: `1711.00937.pdf`

                                    Neural Discrete Representation Learning
                                 AaronvandenOord                 Oriol Vinyals            KorayKavukcuoglu
                                      DeepMind                     DeepMind                    DeepMind
                               avdnoord@google.com           vinyals@google.com           korayk@google.com
                                                                 Abstract
                                 ...
                                 modelthat learns such discrete representations. Our model, the Vector Quantised-
                                 ...
                                 modeltocircumventissuesof“posteriorcollapse”-—wherethelatentsareignored
                                 whentheyarepairedwithapowerfulautoregressivedecoder-—typicallyobserved
                         ...
                         Maximumlikelihood and reconstruction error are two common objectives used to train unsupervised
        ...
        arXiv:1711.00937v2  [cs.LG]  30 May 2018decoder (such as PixelCNN). However, in this paper, we argue for learning discrete and useful latent
                  ...
                  “posterior collapse” issue which has been problematic with many VAE models that have a powerful
                   ...
                   Ourcontributions can thus be summarised as:
                       ...
                       • Weshowthatadiscretelatent model (VQ-VAE) perform as well as its continuous model
                         counterparts in log-likelihood.
                       • Whenpaired with a powerful prior, our samples are coherent and high quality on a wide
                         variety of applications such as speech and video generation.
                       • Weshowevidenceoflearninglanguagethrough raw speech, without any supervision, and
                         showapplications of unsupervised speaker conversion.
                   2  Related Work
                   ...
                   ThereexistmanyalternativesfortrainingdiscreteVAEs. TheNVIL[27]estimatoruseasingle-sample
                   ...
                   Ourworkalsoextends the line of research where autoregressive distributions are used in the decoder
                   of VAEsand/orintheprior[14]. ThishasbeendoneforlanguagemodellingwithLSTMdecoders[4],
                   ...
                   encoding. Other authors [1] propose a method for similar compression model with vector quantisation.
                           ...
                           Wedeﬁnealatentembeddingspacee ∈ RK×D whereK isthesizeofthediscrete latent space (i.e.,
                           a K-way categorical), and D is the dimensionality of each latent embedding vector e . Note that
                                                                                                               i
                           there are K embedding vectors e ∈ RD, i ∈ 1,2,...,K. As shown in Figure 1, the model takes an
                                                           i
                           input x, that is passed through an encoder producing output z (x). The discrete latent variables z
                                                                                       e
                           ...
                           Onecanseethisforwardcomputationpipelineasaregularautoencoderwithaparticularnon-linearity
                           ...
                           Theposterior categorical distribution q(z|x) probabilities are deﬁned as one-hot as follows:
                                                               1 for k=argmin kz (x)−e k ,
                                                 q(z = k|x) =                      j  e       j 2 ,                     (1)
                                                                 0 otherwise
                           where z (x) is the output of the encoder network. We view this model as a VAE in which we
                                  e
                           can bound logp(x) with the ELBO. Our proposal distribution q(z = k|x) is deterministic, and by
                           deﬁning a simple uniform prior over z we obtain a KL divergence constant and equal to logK.
                           Therepresentation z (x) is passed through the discretisation bottleneck followed by mapping onto
                                              e
                           the nearest element of embedding e as given in equations 1 and 2.
                                                  z (x) = e ,   where    k = argmin kz (x)−e k                          (2)
                                                   q        k                       j  e        j 2
                           3.2  Learning
                           Note that there is no real gradient deﬁned for equation 2, however we approximate the gradient
                           similar to the straight-through estimator [3] and just copy gradients from decoder input z (x) to
                                                                                                                   q
                           ...
                           simple estimator worked well for the initial experiments in this paper.
                                                                          3
                     Figure 1: Left: A ﬁgure describing the VQ-VAE. Right: Visualisation of the embedding space. The
                     output of the encoder z(x) is mapped to the nearest point e . The gradient ∇ L (in red) will push the
                                                                 2             z
                     encoder to change its output, which could alter the conﬁguration in the next forward pass.
                     During forward computation the nearest embedding z (x) (equation 2) is passed to the decoder, and
                                                              q
                     ...
                     reconstruction loss.
                     AsseenonFigure1(right), the gradient can push the encoder’s output to be discretised differently in
                     ...
                     gradient estimation of mapping from z (x) to z (x), the embeddings e receive no gradients from
                                                   e      q                 i
                     the reconstruction loss logp(z|z (x)). Therefore, in order to learn the embedding space, we use one
                                             q
                     of the simplest dictionary learning algorithms, Vector Quantisation (VQ). The VQ objective uses
                     the l error to move the embedding vectors e towards the encoder outputs z (x) as shown in the
                         2                              i                        e
                     second term of equation 3. Because this loss term is only used for updating the dictionary, one can
                     alternatively also update the dictionary items as function of moving averages of z (x) (not used for
                                                                                   e
                     ...
                     Thus, the total training objective becomes:
                                                                 2               2
                                   L=logp(x|z (x))+ksg[z (x)]−ek +βkz (x)−sg[e]k ,             (3)
                                              q          e       2     e         2
                     ...
                     Thedecoder optimises the ﬁrst loss term only, the encoder optimises the ﬁrst and the last loss terms,
                     ...
                     Thelog-likelihood of the complete model logp(x) can be evaluated as follows:
                                              logp(x) = logXp(x|z )p(z ),
                                                                 k   k
                                                           k
                     Because the decoder p(x|z) is trained with z = z (x) from MAP-inference, the decoder should not
                                                           q
                     allocate any probability mass to p(x|z) for z 6= z (x) once it has fully converged. Thus, we can write
                                                          q
                                                           4
                           logp(x) ≈ logp(x|z (x))p(z (x)). We empirically evaluate this approximation in section 4. From
                                               q       q
                           Jensen’s inequality, we also can write logp(x) ≥ logp(x|z (x))p(z (x)).
                                                                                   q        q
                           3.3  Prior
                           ...
                           wecangenerate x via ancestral sampling. We use a PixelCNN over the discrete latents for images,
                           ...
                           4   Experiments
                           4.1  Comparisonwithcontinuousvariables
                           Asaﬁrstexperiment we compare VQ-VAEwithnormalVAEs(withcontinuousvariables), as well as
                           VIMCO[28]withindependentGaussianorcategorical priors. We train these models using the same
                           ...
                           3×3blocks(implementedasReLU,3x3conv,ReLU,1x1conv),allhaving256hiddenunits. The
                           ...
                           TheVAE,VQ-VAEandVIMCOmodelsobtain4.51bits/dim,4.67bits/dimand5.14respectively.
                           ...
                           Ourmodelistheﬁrst among those using discrete latent variables which challenges the performance
                           ...
                           4.2  Images
                           Imagescontain a lot of redundant information as most of the pixels are correlated and noisy, therefore
                           ...
                           z = 32×32×1discretespace(withK=512)viaapurelydeconvolutional p(x|z). So a reduction of
                           128×128×3×8 ≈ 42.6 in bits. We model images by learning a powerful prior (PixelCNN) over z. This
                             32×32×9
                           ...
                           latent space, with K=512.
                                                                          5
           Reconstructionsfromthe32x32x1spacewithdiscretelatentsareshowninFigure2. Evenconsidering
           that wegreatlyreducethedimensionalitywithdiscreteencoding,thereconstructionslookonlyslightly
           ...
           PixelCNNweusedwassimilartothoseusedbytheauthorsofthePixelCNNpaper[38].
           Figure 3: Samples (128x128) from a VQ-VAE with a PixelCNN prior trained on ImageNet images.
           Fromleft to right: kit fox, gray whale, brown bear, admiral (butterﬂy), coral reef, alp, microwave,
           pickup.
           ...
           from DeepMind Lab.
           Wealsorepeat the same experiment for 84x84x3 frames drawn from the DeepMind Lab environment
           ...
           sampled from the discretised global code can be seen in Figure 5.
                           ...
                           Weﬁrst consider the VCTK dataset, which has speech recordings of 109 different speakers [41].
                           Wetrain a VQ-VAEwheretheencoderhas6stridedconvolutions with stride 2 and window-size 4.
                           This yields a latent space 64x smaller than the original waveform. The latents consist of one feature
                           mapandthediscrete space is 512-dimensional. The decoder is conditioned on both the latents and a
                           ...
                           showninFigure7,thereconstruction has the same content (same text contents), but the waveform
                           ...
                           Wehavethenanalysedtheunconditionalsamplesfromthemodeltounderstanditscapabilities. Given
                           ...
                           of 40960timesteps(2.56seconds),whichyields320latenttimesteps. Whilesamplesdrawnfromeven
                           ...
                           VQ-VAE).Witha128-dimensionaldiscrete space that runs at 25 Hz (encoder downsampling factor
                                                                                                                          1
                           of 640), we mapped every of the 128 possible latent values to one of the 41 possible phoneme values
                           ...
                           modelfollowedby10framesthataresampledfromVQ-VAEwithallactionssettoforward (toprow)
                           and right (bottom row). Generation of the video sequence with the VQ-VAE model is done purely in
                           the latent space, z without the need to generate the actual images themselves. Each image in the
                                            t
                           sequence xt is then created by mapping the latents with a deterministic decoder to the pixel space
                           after all the latents are generated using only the prior model p(z ,...,z ). Therefore, VQ-VAE can
                                                                                        1      T
                           ...
                           References
                            [1] Eirikur Agustsson, Fabian Mentzer, Michael Tschannen, Lukas Cavigelli, Radu Timofte, Luca Benini, and
                                LucVanGool. Soft-to-hard vector quantization for end-to-end learned compression of images and neural
                           ...
                           [24] Christian Ledig, Lucas Theis, Ferenc Huszár, Jose Caballero, Andrew Cunningham, Alejandro Acosta,
                                AndrewAitken, Alykhan Tejani, Johannes Totz, Zehan Wang, et al. Photo-realistic single image super-
             ...
             [42] Zichao Yang, Zhiting Hu, Ruslan Salakhutdinov, and Taylor Berg-Kirkpatrick. Improved variational
               autoencoders for text modeling using dilated convolutions. CoRR, abs/1702.08139, 2017.
                                  10
                                                A Appendix
                                                A.1        VQ-VAEdictionaryupdateswithExponentialMovingAverages
                                                Asmentioned in Section 3.2, one can also use exponential moving averages (EMA) to update the dictionary
                                                items instead of the loss term from Equation 3:
                                                                                                                                                    2
                                                                                                                          ksg[z (x)] − ek .                                                                                 (4)
                                                                                                                                   e                2
                                                Let {z         , z    , . . . , z      }betheset of n outputs from the encoder that are closest to dictionary item e , so that
                                                           i,1    i,2            i,n                             i                                                                                                 i
                                                                                      i
                                                wecanwritetheloss as:
                                                                                                                            n
                                                                                                                              i
                                                                                                                           X                       2
                                                                                                                                 kz       −ek .                                                                             (5)
                                                                                                                                     i,j        i 2
                                                                                                                             j
                                                Theoptimal value for ei has a closed form solution, which is simply the average of elements in the set:
                                                                                                                                          ni
                                                                                                                           e = 1 Xz .
                                                                                                                            i       n            i,j
                                                                                                                                      i    j
                                                ...
                                                movingaverages as an online version of this update:
                                                                                                             (t)           (t−1)                 (t)
                                                                                                         N :=N                      ∗γ+n (1−γ)                                                                              (6)
                                                                                                            i              i                     i
                                                                                                             (t)           (t−1)               X(t)
